...
SET s.login = CASE WHEN row.time_login IS NOT NULL AND trim(row.time_login) <> '' 
                   THEN datetime(replace(trim(row.time_login), ' ', 'T')) ELSE null END,
    s.last_activity = CASE WHEN row.time_last_activity IS NOT NULL AND trim(row.time_last_activity) <> '' 
                           THEN datetime(replace(trim(row.time_last_activity), ' ', 'T')) ELSE null END
...
SET r.quantity = toInteger(row.quantity),
    ...
    r.arrived = CASE WHEN trim(row.arrived) IS NOT NULL AND trim(row.arrived) <> ''
                THEN datetime(replace(trim(row.arrived), ' ', 'T')) ELSE null END;

// TRUCK IS_MODEL & WAREHOUSE LINKS
LOAD CSV WITH HEADERS FROM 'file:///trucks.csv' AS row
MATCH (t:Truck {id: trim(toString(row.id))})
MERGE (tm:TruckModel {id: trim(toString(row.model_id))})
MERGE (t)-[:IS_MODEL]->(tm)
WITH t, row
CALL {
    WITH t, row
    WITH t, row WHERE row.origin IS NOT NULL AND trim(row.origin) <> ''
...
CALL {
    WITH t, row
    WITH t, row WHERE row.destination IS NOT NULL AND trim(row.destination) <> ''
...
WITH s, row WHERE row.destination IS NOT NULL AND trim(row.destination) <> ''
...
SET r.time_departed = CASE WHEN trim(row.time_departed) IS NOT NULL AND trim(row.time_departed) <> ''
                      THEN datetime(replace(trim(row.time_departed), ' ', 'T')) ELSE null END,
    r.expected_arrival = CASE WHEN trim(row.time_arrived_estimated) IS NOT NULL AND trim(row.time_arrived_estimated) <> ''
                         THEN datetime(replace(trim(row.time_arrived_estimated), ' ', 'T')) ELSE null END;
...
CALL {
    WITH o, row 
    WITH o, row WHERE row.client_id IS NOT NULL AND trim(row.client_id) <> ''
    MATCH (u:User {id: trim(toString(row.client_id))}) MERGE (u)-[:PLACED]->(o)
}
WITH o, row
CALL {
    WITH o, row 
    WITH o, row WHERE row.supplier_id IS NOT NULL AND trim(row.supplier_id) <> ''
    MATCH (s:Supplier {id: trim(toString(row.supplier_id))}) MERGE (s)-[:SUPPLIES_ORDER]->(o)
}
WITH o, row
CALL {
    WITH o, row 
    WITH o, row WHERE row.truck_id IS NOT NULL AND trim(row.truck_id) <> ''
    MATCH (t:Truck {id: trim(toString(row.truck_id))}) MERGE (t)-[:DELIVERING]->(o)
};
...
SET f.cost_fuel = toFloat(row.cost_fuel),
    ...
    f.departure_calculated = CASE WHEN row.calculated IS NOT NULL AND trim(row.calculated) <> '' 
                             THEN datetime(replace(trim(row.calculated), ' ', 'T')) ELSE null END
WITH o, f
